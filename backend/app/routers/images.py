...
def list_images(
    ...
    try:
        ...
        # Try to find an images table
        table_name = None
        for tbl in ["images", "site_images", "media", "image_library"]:
            try:
        ...
        if not table_name:
            return {"data": [], "total": 0, "message": "No images table found"}

        # Discover columns
        cols = [r[0] for r in store_db.execute(text(f"SHOW COLUMNS FROM {table_name}")).fetchall()]

        # Build select
        id_col = next((c for c in cols if c in ["id", "image_id", "img_id"]), cols[0])
        name_col = next((c for c in cols if c in ["name", "filename", "file_name", "image_name", "title"]), None)
        path_col = next((c for c in cols if c in ["path", "file_path", "url", "image_url", "src"]), None)
...
def delete_image(
    ...
    try:
        ...
        if not table_name:
            raise HTTPException(status_code=404, detail="Images table not found")

        cols = [r[0] for r in store_db.execute(text(f"SHOW COLUMNS FROM {table_name}")).fetchall()]
        id_col = next((c for c in cols if c in ["id", "image_id", "img_id"]), cols[0])

        store_db.execute(text(f"DELETE FROM {table_name} WHERE {id_col} = :id"), {"id": image_id})
        store_db.commit()
