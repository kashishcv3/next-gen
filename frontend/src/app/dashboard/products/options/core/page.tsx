...
export default function CoreProductOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/products/options/core');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/products/options/core', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-cogs"></i> Core Product Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure core product settings and behaviors</p>
      </div></div>
