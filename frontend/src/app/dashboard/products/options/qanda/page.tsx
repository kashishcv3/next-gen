...
export default function ProductQandAOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/products/options/qanda');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/products/options/qanda', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-question-circle"></i> Product Q&A Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure product Q&A settings</p>
      </div></div>
