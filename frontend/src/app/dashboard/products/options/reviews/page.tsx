...
export default function ProductReviewOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/products/options/reviews');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/products/options/reviews', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-star"></i> Product Review Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure product review settings</p>
      </div></div>
