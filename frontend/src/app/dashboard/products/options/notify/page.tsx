...
export default function ProductNotificationOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/products/options/notify');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/products/options/notify', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-bell"></i> Product Notification Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure product notification settings</p>
      </div></div>
