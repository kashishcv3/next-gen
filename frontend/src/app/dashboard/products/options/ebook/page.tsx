...
export default function EbookOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/products/options/ebook');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/products/options/ebook', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-book"></i> Ebook Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure ebook product settings</p>
      </div></div>
