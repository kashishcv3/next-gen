...
export default function InventoryControlOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/products/options/inventory');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/products/options/inventory', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-database"></i> Inventory Control Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure inventory management settings</p>
      </div></div>
