...
export default function AutomaticRemindersPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/auto-reminders');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/auto-reminders', options);
      setSuccess('Automatic Reminders saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Automatic Reminders</h1>
        <p><i className="fa fa-bell"></i> Configure automatic reminder notification settings</p>
      </div></div>
