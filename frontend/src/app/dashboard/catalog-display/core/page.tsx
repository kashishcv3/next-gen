...
export default function CatalogDisplayOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/store/catalog-display/core');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/store/catalog-display/core', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-th"></i> Catalog Display Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure product catalog display settings</p>
      </div></div>
