...
export default function CoreDisplayOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/store/display-options/core');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/store/display-options/core', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-eye"></i> Core Display Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure core store display settings</p>
      </div></div>
