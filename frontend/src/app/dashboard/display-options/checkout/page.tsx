...
export default function CheckoutDisplayOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/store/display-options/checkout');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/store/display-options/checkout', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-shopping-cart"></i> Checkout Display Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure checkout page display settings</p>
      </div></div>
