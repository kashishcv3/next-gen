...
export default function SiteSearchOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/store/display-options/search');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/store/display-options/search', options);
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1><i className="fa fa-search"></i> Site Search Options</h1>
        <p><i className="fa fa-info-circle"></i> Configure site search functionality</p>
      </div></div>
