...
export default function WholesaleEditPage() {
  ...
  const [formData, setFormData] = useState<WholesaleData>({
    id: '',
    company_name: '',
    contact_name: '',
    email: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/wholesale/${id}`, formData);
      setSuccess(true);
      ...
      console.error('Failed to update wholesale:', err);
      setError('Failed to update wholesale customer');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="container-fluid" style={{ padding: '20px' }}>
      <h1>Edit Wholesale Customer</h1>

      {error && <div className="alert alert-danger">{error}</div>}
      {success && <div className="alert alert-success">Wholesale customer updated successfully!</div>}
