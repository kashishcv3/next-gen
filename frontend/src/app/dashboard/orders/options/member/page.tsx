...
export default function MemberOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/orders/options/member');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Member Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        ...
        <p><i className="fa fa-users"></i> Configure member-related settings and options</p>
      </div></div>
