...
export default function CartAbandonmentOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/growth-options/abandon');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/growth-options/abandon', options);
      setSuccess('Cart Abandonment Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Cart Abandonment Options</h1>
        <p><i className="fa fa-shopping-cart"></i> Configure cart abandonment recovery campaign settings</p>
      </div></div>
