...
export default function EmailCampaignOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/growth-options/email');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/growth-options/email', options);
      setSuccess('Email Campaign Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Email Campaign Options</h1>
        <p><i className="fa fa-envelope"></i> Configure email marketing growth campaign settings</p>
      </div></div>
