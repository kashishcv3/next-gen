...
export default function ListPage() {
  ...
  const fetchItems = async (searchTerm?: string) => {
    try {
      ...
      const response = await api.get(`/promos?${params.toString()}`);
        ...
        {items.length > 0 ? (
          <div className="table-responsive">
            <table className="table table-striped">
              ...
              <tbody>{items.map(item => (<tr key={item.id}><td>{item.name || item.title}</td><td>{item.status || 'N/A'}</td><td>{item.created_at ? formatDate(item.created_at) : 'N/A'}</td><td><Link href={`/promos/${item.id}`} className="btn btn-xs btn-primary">View</Link></td></tr>))}</tbody>
            </table>
          </div>
        ) : (
