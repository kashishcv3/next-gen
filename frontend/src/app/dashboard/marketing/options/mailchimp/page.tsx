...
export default function MailchimpOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/mailchimp');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/mailchimp', options);
      setSuccess('Mailchimp Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Mailchimp Options</h1>
        <p><i className="fa fa-envelope"></i> Configure Mailchimp email marketing integration settings</p>
      </div></div>
