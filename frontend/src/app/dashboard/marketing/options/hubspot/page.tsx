...
export default function HubspotOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/hubspot');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/hubspot', options);
      setSuccess('Hubspot Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Hubspot Options</h1>
        <p><i className="fa fa-bullhorn"></i> Configure Hubspot marketing automation integration settings</p>
      </div></div>
