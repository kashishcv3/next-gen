...
export default function StructuredDataOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/structured-data');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/structured-data', options);
      setSuccess('Structured Data Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Structured Data Options</h1>
        <p><i className="fa fa-code"></i> Configure structured data markup settings for SEO</p>
      </div></div>
