...
export default function KlaviyoOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/klaviyo');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/klaviyo', options);
      setSuccess('Klaviyo Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Klaviyo Options</h1>
        <p><i className="fa fa-envelope"></i> Configure Klaviyo email marketing integration settings</p>
      </div></div>
