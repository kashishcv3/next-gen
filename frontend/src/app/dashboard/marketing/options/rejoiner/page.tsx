...
export default function RejoinOptionPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/rejoiner');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/rejoiner', options);
      setSuccess('Rejoiner Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Rejoiner Options</h1>
        <p><i className="fa fa-envelope"></i> Configure Rejoiner email marketing integration settings</p>
      </div></div>
