...
export default function BrontoOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/bronto');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/bronto', options);
      setSuccess('Bronto Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>Bronto Options</h1>
        <p><i className="fa fa-envelope"></i> Configure Bronto email marketing integration settings</p>
      </div></div>
