...
export default function DotmailerOptionsPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await api.get('/marketing/options/dotmailer');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/marketing/options/dotmailer', options);
      setSuccess('dotmailer Options saved successfully');
    } catch (err: any) {
  ...
  return (
    <div className="container-fluid" style={{padding:'20px'}}>
      <div className="row"><div className="col-lg-12">
        <h1>dotmailer Options</h1>
        <p><i className="fa fa-envelope"></i> Configure dotmailer email marketing integration settings</p>
      </div></div>
